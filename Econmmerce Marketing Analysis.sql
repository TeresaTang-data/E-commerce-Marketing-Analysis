...
SELECT YEAR(w.created_at) AS yr,
       ...
       COUNT(DISTINCT o.order_id)/COUNT(DISTINCT w.website_session_id) AS order_to_session_conversion_rate
...
SELECT YEAR(created_at), MONTH(created_at),
               ...
               SUM(CASE WHEN product_id = 1 THEN price_usd ELSE NULL END)/SUM(price_usd) AS revenue_percentage
FROM order_items
GROUP BY 1,2
ORDER BY 1,2
...
CREATE TEMPORARY TABLE product_pageview AS(
